...
Write the absolute path of the located repo to `~/.config/kilo/.pack-repo` (single line, no trailing newline). This lets `/sync-pack` find the repo later regardless of clone location.

## Step 7 — Verify and report

Confirm all seven rule files are present in the config `rules/` and registered in `instructions`; confirm `command/graphify.md`, `command/sync-pack.md`, and `command/install-pack.md` exist in the config; confirm `skills.paths` points at the config `skills` folder. Summarize: repo location used, marker written, backup location, folders copied, rules registered, skills.paths status. Tell the user to start a new Kilo session or run `/reload`.
